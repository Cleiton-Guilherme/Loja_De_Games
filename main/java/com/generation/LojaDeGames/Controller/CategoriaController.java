...
public class CategoriaController {
	...
	@GetMapping("/{id}")
	public ResponseEntity<CategoriaController> getById(@PathVariable long id){
		return repository.findById(id).map(resp -> ResponseEntity.ok(resp)).orElse(ResponseEntity.notFound().build());
		
	}
	
	@GetMapping("/categoria/{categoria}")
	public ResponseEntity<List<CategoriaController>> getByName(@PathVariable String categoria){
